...
is_allowed_shared_owner() {
  local skill_name="$1"
  local owners="$2"

  case "$skill_name:$owners" in
    "accessibility:GENERIC_UI.md,WORKFLOW.md")
      return 0
      ;;
    "rust:BACKEND.md,TAURI.md")
      return 0
      ;;
    "unit-testing:GENERIC_UI.md,WORKFLOW.md")
      return 0
      ;;
    *)
      return 1
...
while IFS= read -r ref; do
  ...
  if [ ! -f "$full_path" ]; then
    echo "  [ERROR] AGENTS.md references '$ref' but $full_path does not exist"
    errors=$((errors + 1))
  else
    echo "  [OK] $ref"
  fi
...
while IFS= read -r routing_file; do
  ...
  if [ -z "$parent_ref" ]; then
    echo "  [ERROR] ORPHAN ROUTING FILE: $rel_path is not referenced from AGENTS.md or any routing branch"
    errors=$((errors + 1))
  fi
...
while IFS= read -r ref; do
  full_path="$AGENTS_DIR/$ref"
  if [ ! -f "$full_path" ]; then
    echo "  [ERROR] Referenced '$ref' but $full_path does not exist"
    errors=$((errors + 1))
...
if [ "$check2_errors" -eq 0 ]; then
  echo "  [OK] Found $ref_count leaf references, all resolved."
else
  echo "  [ERROR] Found $check2_errors unresolved leaf reference(s) out of $ref_count."
fi
echo ""
...
while IFS= read -r skill_path; do
  ...
  if [ -z "$found" ]; then
    echo "  [ERROR] ORPHAN: $skill_name exists on disk but is not referenced in any routing file"
    errors=$((errors + 1))
  fi
...
while IFS= read -r skill_path; do
  ...
  if [ "$count" -gt 1 ]; then
    owners=$(printf "%s\n" "$owner_files" | sed '/^$/d' | sed "s|$ROUTING_DIR/||" | sort | paste -sd, -)
    if is_allowed_shared_owner "$skill_name" "$owners"; then
      echo "  [OK] $skill_name shared intentionally across: $owners"
      allowed_shared=$((allowed_shared + 1))
    else
      echo "  [WARN] $skill_name appears in $count files: $owners"
      warnings=$((warnings + 1))
    fi
  fi
done < <(find "$SKILLS_DIR" -name "SKILL.md" -not -path "*/routing/*" | sort)
...
while IFS= read -r routing_file; do
  while IFS= read -r ref; do
    full_path="$AGENTS_DIR/$ref"
    if [ ! -f "$full_path" ]; then
      echo "  [ERROR] $(basename "$routing_file") references '$ref' but $full_path does not exist"
      errors=$((errors + 1))
    fi
...
while IFS= read -r skill_file; do
  ...
  if [ "$has_name" -eq 0 ] || [ "$has_desc" -eq 0 ]; then
    echo "  [WARN] $skill_name: missing frontmatter (name=$has_name, description=$has_desc)"
    warnings=$((warnings + 1))
  fi
done < <(find "$SKILLS_DIR" -name "SKILL.md" -not -path "*/routing/*" | sort)
echo "  Done."
...
echo "Allowed shared ownership: $allowed_shared"
if [ "$errors" -eq 0 ] && [ "$warnings" -eq 0 ]; then
  echo "[OK] All checks passed. Routing tree is valid."
elif [ "$errors" -eq 0 ]; then
  echo "[WARN] $warnings warning(s), 0 errors. Tree is valid but review warnings."
else
  echo "[ERROR] $errors error(s), $warnings warning(s). Tree has issues that must be fixed."
  exit 1
fi
