...
description: UI verification criteria, structure checklists, severity definitions, and tolerance rules for comparing implementations against approved design specifications. Use for verifying UI matches design, understanding what to check, and determining acceptable differences.
---

# UI Verification

This skill provides criteria and guidelines for verifying that UI implementations match approved design specifications. It defines what to verify, severity levels, and acceptable tolerances.

## When to Use

- When verifying UI implementation against approved design specifications
...
| Check                   | Description                                              |
| ----------------------- | -------------------------------------------------------- |
| **Container hierarchy** | Does DOM structure match the design layer hierarchy?      |
| **Nesting depth**       | Are elements nested at the same level as in the design?   |
...
| **Sections present**    | Are ALL sections from the design present in implementation? |
...
| **Card/panel boundaries**    | Does card have same width as in the design?  |
...
**CRITICAL**: If a card/container in the design is narrow and centered, but implementation shows it wider or full-width, this MUST be reported.
...
- [ ] Design specification available for the component/view
...
- [ ] **All sections from the design are present** in implementation
- [ ] Container hierarchy matches design layers
...
| Issue   | Expected (Design Spec) | Actual (Implementation) |
| ------- | ---------------- | ----------------------- |
...
| Property | Expected (Design Spec) | Actual (Implementation) | Severity               |
| -------- | ---------------- | ----------------------- | ---------------------- |
...
| ---------- | ------------------------------------------- | ----------------------------------------------------- |
| **HIGH**   | Both design and implementation data complete | Trust the report, fix all differences                 |
