...
When using this skill standalone (or when the user explicitly requests a separate codebase-analysis report), save the analysis results as a standalone document following the `./references/codebase-analysis.example.md` template.
When this skill is invoked by another workflow command that defines its own required output file/template, embed the relevant findings into that caller's required output instead of creating a separate `codebase-analysis` report, unless the user explicitly asks for one.

For standalone reports following `./references/codebase-analysis.example.md`, don't add or remove any sections from the template. Follow the structure and naming conventions strictly to ensure clarity and consistency.
