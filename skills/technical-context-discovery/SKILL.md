...
```
Discovery progress:
- [ ] Step 1: Check project instruction files
...
If no project instructions are found, or if they don't cover specific aspects, **analyze the existing codebase** to understand and replicate established patterns:
...
If neither project instructions nor sufficient existing codebase patterns are available (e.g., new project, greenfield feature, or first implementation of a specific pattern), **use external documentation and industry best practices**:

- Use available documentation search tools to find official framework and library docs (check project config for exact versions first).
